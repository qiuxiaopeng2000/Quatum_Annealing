...
for name in names_FSP:
    ...
    sa_result.load(evaluate=False, single_flag=True)
    ex_result = MethodResult('soqp', problem_result.path, problem)
    ex_result.load(evaluate=False, single_flag=True)
    ...
    table = Visualizer.tabulate_single_problem(
        name, ['sa', 'soqp'], ['time', 'statistic', 'p_value', 'mean', 'std', 'max', 'min'],
...
for name in names_NRP:
    ...
    # compare
    scores = problem_result.statistical_analysis(method1="sa", method2="soqp", weights=weight_NRP)
    table = Visualizer.tabulate_single_problem(
        name, ['sa', 'soqp'], ['time', 'statistic', 'p_value', 'mean', 'std', 'max', 'min'],
        scores, {'time': 6, 'statistic': 12, 'p_value': 18, 'mean': 4, 'std': 4, 'max': 4, 'min': 4}
    )
