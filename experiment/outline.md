...
2. 使用基于最大能量影响的方法（只选取部分变量、每次一个子问题、下一个循环返回下一个子问题，若真个问题都搜寻完了且未跳出循环则从头重新来，若该子问题包含的变量发生改变则取代该变量在初始解中的值作为本次循环的量子退火解）与classic算法（从初始解开始搜寻）并行求解，选取能量最小结果，然后交给tabu进行局部搜索得到一个循环的结果。
