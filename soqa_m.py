...
# names_FSP = ['BerkeleyDB', 'ERS', 'WebPortal', 'Drupal']
# names_FSP = ['Amazon', 'E-Shop', 'eCos', 'uClinux']
names_FSP = ['Amazon', 'E-Shop']
...
for name in names_NRP:
    ...
    for _ in range(1):
        result = QAWSOSolver.solve_best(problem=problem, iterations=30, num_reads=100, penalty=penalty, weights=weight_NRP)
...
for name in names_FSP:
    ...
    penalty = EmbeddingSampler.calculate_penalty(wso, problem.constraint_sum)
    for _ in range(1):
        result = QAWSOSolver.solve_best(problem=problem, iterations=30, num_reads=100, penalty=penalty, weights=weight_FSP)
        moqa_method_result.add(result)
