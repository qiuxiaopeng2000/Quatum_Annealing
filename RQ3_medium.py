...
for name in names_NRP:
    ...
    nsgaii_result.load()
    moqa_result = MethodResult('moqa', moqa_problem_result.path, problem)
    moqa_result.load()
    ...
    soqa_result.load(single_flag=True)
    ...
    hymoo_problem_result.add(moqa_result)
    hymoo_problem_result.add(hymoo_result)

    hysoo_problem_result.add(hysoo_result)
    hysoo_problem_result.add(soqa_result)

    hysoo_problem_result.add(sa_result)

    # compare
    moo_methods = ['nsgaii', 'moqa', 'hymoo']
    scores_moo = hymoo_problem_result.average_list_compare(methods=moo_methods)
    table_soqa = Visualizer.tabulate_single_problem(
        name, [method for method in moo_methods], ['elapsed time', 'found', 'front', 'igd', 'hv', 'spacing', 'tts'],
        scores_moo, {'elapsed time': 4, 'found': 5, 'front': 4, 'igd': 4, 'hv': 4, 'spacing': 4, 'tts': 4}
    )
    Visualizer.tabluate(table_soqa, 'soqa-hymoo-{}.csv'.format(name))
...
for name in names_FSP:
    ...
    table_hymoo = Visualizer.tabulate_single_problem(
        name, ['moqa', 'hymoo'], ['elapsed time', 'found', 'front', 'igd', 'hv', 'spacing', 'tts'],
        scores_hymoo, {'elapsed time': 4, 'found': 5, 'front': 4, 'igd': 4, 'hv': 4, 'spacing': 4, 'tts': 4}
    )
    ...
    scores_soqa = hysoo_problem_result.statistical_analysis(method1="hysoo", method2="soqa", weights=weight_FSP)
    table_soqa = Visualizer.tabulate_single_problem(
        name, ['hysoo', 'soqa'], ['time', 'statistic', 'p_value', 'mean', 'std', 'max', 'min'],
        scores_soqa, {'time': 6, 'statistic': 12, 'p_value': 18, 'mean': 4, 'std': 4, 'max': 4, 'min': 4}
    )
    Visualizer.tabluate(table_soqa, 'soqa-hysoo-compare-{}.csv'.format(name))
